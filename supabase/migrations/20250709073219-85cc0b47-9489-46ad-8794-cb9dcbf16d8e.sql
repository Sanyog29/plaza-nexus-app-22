-- Phase 1: Complete Backend Integration - Fixed Migration
...
-- 3. Staff skills matrix for intelligent assignment (use user_id instead of staff_id)
...
-- 5. Staff workload analytics for optimization (use user_id instead of staff_id)
...
CREATE POLICY "Staff can view their skills" ON public.staff_skills FOR SELECT USING ((auth.uid() = user_id) OR is_admin(auth.uid()));
CREATE POLICY "Staff can update their skills" ON public.staff_skills FOR ALL USING ((auth.uid() = user_id) OR is_admin(auth.uid()));
...
CREATE POLICY "Staff can view their own analytics" ON public.staff_workload_analytics FOR SELECT USING (auth.uid() = user_id);
...
CREATE INDEX IF NOT EXISTS idx_staff_skills_user_id ON public.staff_skills(user_id);
CREATE INDEX IF NOT EXISTS idx_workload_analytics_user_date ON public.staff_workload_analytics(user_id, analysis_date);
...
INSERT INTO public.staff_skills (user_id, skill_category, skill_level, certified, certification_expiry)
SELECT 
  p.id,
  skill,
  CASE 
...
-- Insert workload analytics for staff
INSERT INTO public.staff_workload_analytics (
  user_id, analysis_date, active_requests, completed_requests, 
  avg_completion_time_hours, workload_score, efficiency_rating
)
SELECT 
  p.id,
  date_series.analysis_date,
  (random() * 10)::integer,
  (random() * 8)::integer,
  (2 + random() * 8)::numeric(8,2),
  (50 + random() * 50)::integer,
  (0.6 + random() * 0.4)::numeric(3,2)
FROM profiles p
CROSS JOIN generate_series(CURRENT_DATE - interval '29 days', CURRENT_DATE, interval '1 day') AS date_series(analysis_date)
WHERE p.role IN ('admin', 'ops_supervisor', 'field_staff')
