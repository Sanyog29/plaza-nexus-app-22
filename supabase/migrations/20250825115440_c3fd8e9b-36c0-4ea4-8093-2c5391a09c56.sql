...
AS $$
DECLARE
  work_time TIMESTAMP WITH TIME ZONE;
...
BEGIN
  ...
  IF work_time::TIME < day_start THEN
    work_time := work_time::DATE + day_start;
  ELSIF work_time::TIME >= day_end THEN
    work_time := (work_time::DATE + INTERVAL '1 day') + day_start;
  END IF;
  
  -- Skip weekends
  WHILE EXTRACT(DOW FROM work_time) IN (0, 6) LOOP
    work_time := (work_time::DATE + INTERVAL '1 day') + day_start;
  END LOOP;
  
  WHILE remaining_hours > 0 LOOP
    -- Calculate hours available today
    end_of_day := work_time::DATE + day_end;
    hours_today := LEAST(
      remaining_hours,
      EXTRACT(EPOCH FROM (end_of_day - work_time))/3600
    )::INTEGER;
