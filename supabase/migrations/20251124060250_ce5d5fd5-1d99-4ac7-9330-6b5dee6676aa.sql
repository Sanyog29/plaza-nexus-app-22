-- Update admin_create_user_invitation to handle unassigned property default
DROP FUNCTION IF EXISTS public.admin_create_user_invitation(text, text, text, text, text, text, text, text, text, uuid, text, text);
...
BEGIN
  ...
  -- Use provided property_id or default to "Unassigned"
  final_property_id := COALESCE(invitation_property_id, unassigned_property_id);
  
  -- Generate invitation token
  invitation_token := gen_random_uuid();
  
  -- Insert invitation with property_id (always has a value now)
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'is_default_property', (final_property_id = unassigned_property_id),
      'invited_by', auth.uid()
    )
  );
  ...
  RETURN jsonb_build_object(
    'success', true,
    'message', CASE 
      WHEN final_property_id = unassigned_property_id 
      THEN 'User invitation created successfully. User will be assigned to "Unassigned" property - please assign a real property after approval.'
      ELSE 'User invitation created successfully'
    END,
