...
ADD COLUMN assigned_group UUID REFERENCES public.staff_groups(id),
ADD COLUMN assignment_acknowledged_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN is_crisis BOOLEAN NOT NULL DEFAULT false,
ADD COLUMN escalation_level INTEGER NOT NULL DEFAULT 1 CHECK (escalation_level BETWEEN 1 AND 5),
ADD COLUMN next_escalation_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN auto_assignment_attempts INTEGER NOT NULL DEFAULT 0;
