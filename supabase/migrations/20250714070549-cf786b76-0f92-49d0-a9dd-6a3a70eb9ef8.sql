-- Fix missing user profiles and strengthen the user management system
...
-- Create a notification function for system health issues
CREATE OR REPLACE FUNCTION public.notify_system_health_issues()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  SELECT public.get_system_health_info() INTO health_info;
  
  orphaned_count := (health_info->>'orphaned_users')::integer;
  
  -- If there are orphaned users, notify all admins
  IF orphaned_count > 0 THEN
    ...
    LOOP
      INSERT INTO public.notifications (
        ...
        admin_user_id,
        'System Health Alert',
        format('Found %s users without profiles. Please run the repair function.', orphaned_count),
        'warning',
        '/admin/users'
      );
    END LOOP;
