...
-- Recreate admin_get_vendor_staff_assignments with enhanced orphaned record handling
...
BEGIN
  ...
  SELECT 
    ...
    CASE WHEN p.id IS NULL THEN true ELSE false END as is_orphaned
  FROM vendor_staff vs
  JOIN vendors v ON vs.vendor_id = v.id
  LEFT JOIN profiles p ON vs.user_id = p.id  -- LEFT JOIN to show orphaned records
  ORDER BY 
    CASE WHEN p.id IS NULL THEN 0 ELSE 1 END, -- Show orphaned records at top
    v.name, 
