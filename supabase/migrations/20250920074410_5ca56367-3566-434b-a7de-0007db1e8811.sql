...
DROP FUNCTION IF EXISTS public.admin_get_vendor_staff_assignments();
DROP FUNCTION IF EXISTS public.admin_get_unassigned_users();

-- Recreate enhanced functions with auth.users fallback

-- 1. Create admin_get_vendor_staff_assignments with auth.users fallback
...
-- 2. Create admin_get_unassigned_users with auth.users fallback
...
-- 4. Update cleanup function to backfill first, then cleanup orphaned records
