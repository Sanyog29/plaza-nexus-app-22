-- Fix ON CONFLICT errors by ensuring unique constraints exist and removing duplicates first
DO $$
BEGIN
  ...
  IF to_regclass('public.analytics_summaries') IS NOT NULL THEN
    -- Remove duplicates (keep latest by calculated_at; tie-breaker ctid)
    WITH ranked AS (
      SELECT ctid,
             ROW_NUMBER() OVER (
  ...
  IF to_regclass('public.user_performance_scores') IS NOT NULL THEN
    -- Remove duplicates (prefer latest by updated_at/created_at; tie-breaker ctid)
    WITH ranked AS (
      SELECT ctid,
             ROW_NUMBER() OVER (
  ...
  IF to_regclass('public.vendor_analytics') IS NOT NULL THEN
    -- Remove duplicates (prefer latest by created_at; tie-breaker ctid)
    WITH ranked AS (
      SELECT ctid,
             ROW_NUMBER() OVER (
  ...
  IF to_regclass('public.dietary_preferences') IS NOT NULL THEN
    -- Remove duplicates (prefer latest by created_at/updated_at; tie-breaker ctid)
    WITH ranked AS (
      SELECT ctid,
             ROW_NUMBER() OVER (
