-- Phase 1A: Drop all triggers on profiles table
DROP TRIGGER IF EXISTS profile_role_change_audit ON public.profiles;
DROP TRIGGER IF EXISTS log_sensitive_operation_trigger ON public.profiles;
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Drop specific policies that reference role column
