...
-- Insert sample ML predictions for testing
...
(
  (SELECT id FROM ml_models WHERE model_name = 'Escalation Predictor' LIMIT 1),
  'escalation_risk',
  'maintenance_request',
  (SELECT id FROM maintenance_requests WHERE status = 'pending' LIMIT 1),
  '{"escalation_probability": 0.73, "risk_level": "high"}'::jsonb,
  0.73,
  '{"factors": ["priority", "response_time", "complexity"], "recommended_actions": ["immediate_assignment", "additional_resources"]}'::jsonb
),
(
  (SELECT id FROM ml_models WHERE model_name = 'Demand Forecaster' LIMIT 1),
  'demand_forecast',
  'facility',
  'building-main',
  '{"next_week_requests": 45, "peak_days": ["Monday", "Wednesday"], "resource_needs": {"staff": 6, "equipment": 3}}'::jsonb,
  0.89,
  '{"forecast_period": "7_days", "seasonal_factors": true, "confidence_interval": "85-95%"}'::jsonb
),
(
  (SELECT id FROM ml_models WHERE model_name = 'Anomaly Detector' LIMIT 1),
  'anomaly_detection',
  'asset',
  (SELECT id FROM assets LIMIT 1),
  '{"anomaly_score": 0.23, "status": "normal", "trends": ["stable_performance", "normal_usage"]}'::jsonb,
  0.91,
  '{"monitored_metrics": ["temperature", "vibration", "power_consumption"], "baseline_period": "30_days"}'::jsonb
);
...
(
  ...
  '{"trigger_type": "maintenance_overdue", "request_id": "' || (SELECT id FROM maintenance_requests WHERE status = 'pending' LIMIT 1) || '", "overdue_hours": 24}'::jsonb,
  'completed',
  '[
