-- Enhanced ticket management system implementation (fixed)

-- Create staff group types enum
CREATE TYPE IF NOT EXISTS staff_group_type AS ENUM ('mst_field', 'housekeeping', 'security');

-- Create staff availability status enum  
CREATE TYPE IF NOT EXISTS availability_status_type AS ENUM ('available', 'busy', 'offline', 'on_leave');

-- Create SLA priority enum with specific timings
CREATE TYPE IF NOT EXISTS sla_priority_type AS ENUM ('critical', 'high', 'medium', 'low');
