...
drop policy if exists "Public read vendor-qr-codes" on storage.objects;
create policy "Public read vendor-qr-codes"
  on storage.objects for select
  using (bucket_id = 'vendor-qr-codes');

-- Vendor staff can upload QR to their own vendor folder (first path segment = vendor_id)
drop policy if exists "Vendor staff can upload vendor QR" on storage.objects;
create policy "Vendor staff can upload vendor QR"
...
drop policy if exists "Vendor staff can update vendor QR" on storage.objects;
create policy "Vendor staff can update vendor QR"
...
drop policy if exists "Vendor staff can delete vendor QR" on storage.objects;
create policy "Vendor staff can delete vendor QR"
