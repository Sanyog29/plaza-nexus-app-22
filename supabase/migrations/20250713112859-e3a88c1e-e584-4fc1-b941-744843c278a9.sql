-- Insert sample maintenance requests with various statuses
DO $$
DECLARE
...
BEGIN
    ...
    ('Water Leak in Restroom', 'Small water leak detected under sink in mens restroom', 'urgent', 'assigned', 'Building A - 2nd Floor Mens Restroom', category_plumbing_id, tenant_user_id, staff_user_id, NOW() + INTERVAL '1 hour', NOW() - INTERVAL '30 minutes', NOW() - INTERVAL '15 minutes', NOW() + INTERVAL '2 hours'),
    ...
    ('Elevator Noise', 'Elevator making strange sounds between 2nd and 3rd floor', 'low', 'assigned', 'Building A - Elevator', category_general_id, tenant_user_id, staff_user_id, NOW() + INTERVAL '12 hours', NOW() - INTERVAL '4 hours', NOW() - INTERVAL '4 hours', NOW() + INTERVAL '16 hours'),
