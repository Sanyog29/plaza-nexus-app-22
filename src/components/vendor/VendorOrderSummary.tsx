...
Thank you for your business!
  ...
  return <div className="w-96 bg-background border rounded-lg flex flex-col h-fit">
      ...
      <div className="flex-1 overflow-y-auto p-4 max-h-96">
        ...
        <div className="space-y-3 mb-6">
          {cartItems.map(item => <div key={item.id} className="flex items-center space-x-3 p-3 bg-muted/50 rounded-lg">
              <img src="/placeholder.svg" alt={item.name} className="w-12 h-12 rounded-lg object-cover" />
      ...
      <div className="p-4 border-t space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div>
            ...
            <Input type="number" value={customDiscount} onChange={e => setCustomDiscount(Number(e.target.value))} className="h-8 text-xs" step="0.1" />
          </div>
        </div>
        ...
        <div className="space-y-1 text-sm">
          ...
          {totalDiscount > 0 && <div className="flex justify-between text-green-600">
        ...
        {/* Auto Discount Info */}
        {autoDiscount > 0 && <div className="bg-green-50 p-2 rounded-lg">
        ...
        <div className="space-y-2">
          <Button className="w-full" onClick={handleConfirmPayment} disabled={cartItems.length === 0}>
            <CreditCard className="w-4 h-4 mr-2" />
            Confirm Payment - ₹{total.toFixed(2)}
          </Button>

          {lastOrder && <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={generateReceipt} className="flex-1">
