...
import { Coffee, Sandwich, Cookie, Salad, Pizza, IceCream, Wine, MoreHorizontal, Grid3X3 } from 'lucide-react';
...
export const ModernCategoryTabs: React.FC<ModernCategoryTabsProps> = ({
  ...
  const allItemsCount = categories.reduce((total, category) => total + getCategoryItemCount(category.id), 0);
  return <div className="bg-card border-b border-border px-4 py-4 mx-[5px] my-[5px]">
      <ScrollArea className="w-full">
        <div className="flex gap-2 pb-2">
          {/* All Menu Tab */}
          <Button variant={selectedCategory === "all" ? "default" : "ghost"} size="sm" onClick={() => onCategoryChange("all")} className="flex items-center gap-2 whitespace-nowrap rounded-full">
          ...
          return <Button key={category.id} variant={selectedCategory === category.id ? "default" : "ghost"} size="sm" onClick={() => onCategoryChange(category.id)} className="flex items-center gap-2 whitespace-nowrap rounded-full">
