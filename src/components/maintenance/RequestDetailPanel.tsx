...
const RequestDetailPanel: React.FC<RequestDetailPanelProps> = ({
  ...
  const fetchStaff = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('profiles').select('*').in('role', ['ops_supervisor', 'admin']);
      if (error) throw error;
      setStaff(data || []);
    } catch (error: any) {
  ...
  return <Card className="bg-card/50 backdrop-blur mt-4">
      ...
      {expanded && <CardContent>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {slaInfo && <div className="col-span-2">
                  <div className="flex justify-between items-center mb-1">
                    <p className="text-sm text-gray-400">SLA Status</p>
                    <Badge className={`${slaInfo.breached ? 'bg-red-900 text-red-300' : 'bg-yellow-900 text-yellow-300'}`} variant="outline">
                      {slaInfo.breached ? <AlertTriangle size={12} className="mr-1" /> : <Timer size={12} className="mr-1" />}
                      {slaInfo.timeLeft}
                    </Badge>
                  </div>
                  <Progress value={slaInfo.percentage} className="h-2" indicatorClassName={slaInfo.percentage < 25 ? "bg-red-600" : "bg-yellow-600"} />
                </div>}
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 pt-4 mt-4 border-t border-gray-700">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select value={updatedPriority || undefined} onValueChange={value => setUpdatedPriority(value as RequestPriority)} disabled={updating}>
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select value={updatedAssignee || 'unassigned'} onValueChange={value => setUpdatedAssignee(value === 'unassigned' ? null : value)} disabled={updating}>
                  <SelectTrigger className="bg-card border-gray-700">
              ...
              {updatedStatus !== request.status && <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-white">Status Update Note</label>
                  <Textarea placeholder="Add a note about this status change..." value={statusNote} onChange={e => setStatusNote(e.target.value)} className="bg-card border-gray-700" disabled={updating} />
                </div>}
              
              <div className="flex justify-end mt-2">
                <Button onClick={handleUpdate} disabled={updating} className="bg-plaza-blue hover:bg-blue-700">
